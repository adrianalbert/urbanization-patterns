...
class City():
    ...
    def compute_regions(self, c=0):
        ...
        for x,y in zip(s,c):
            ...
            areas_distr=compute_patch_area_distribution(regions, mask_regions)
            self.regions[x], self.masks_regions[x], self.areas_distr[x] = regions, mask_regions, areas_distr
        return self.regions
    ...
    def compute_profile(self, c=None, loc=None, **kwargs):
        ...
        s = [self.sources[x] for x in c] if hasattr(self, 'sources') else c
        for x,y in zip(s,c):
            theta, rays = extract_rays(self.M[:,:,y], x0, y0, **kwargs)
            rays_mu = np.abs(rays).mean(0); 
            scale   = rays_mu.max() + 1e-5; 
            rays_mu = rays_mu / scale
            rays_se = np.abs(rays).std(0); 
